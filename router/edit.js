...
router.get("/label", (req, res) => {
    var sql = `select * from travel_label`;
    pool.query(sql, (err, result) => {
        if (err) throw err;
        res.send(result);
    })
...
router.post("/pub", (req, res) => {
    var obj = req.body;
    pool.query("INSERT INTO travel_indexnew SET ?", [obj], function (err, result) {
        if (err) throw err;
...
router.post("/drafts", (req, res) => {
    var obj = req.body;
    pool.query("INSERT INTO travel_drafts SET ?", [obj], function (err, result) {
        if (err) throw err;
        if (result.affectedRows == 1) {
            res.send("保存成功")
        }
...
router.get("/editdrafts", (req, res) => {
    var id=req.query.lid;
    var sql = `select * from travel_drafts where id = ?`;
    pool.query(sql,[id], (err, result) => {
        if (err) throw err;
        res.send(result);
    })
                ...
                for (item in files) {
                    ...
                    fs.rename(tempfilepath, filenewpath, function (err) {
                        ...
                        if (err) {
                            ...
                            // 拼接图片url地址
                            result = 'http://gotravel.applinzi.com/' + '/' + filename;
                            console.log(result);
                        }
                        // 返回结果
